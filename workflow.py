...
class pipeline:
    ...
    def __delete_path(self, paths):
        ...
        for a_path in paths:
            ...
            if self.__isprotected(a_path):
                temp = "==> Protected path (will not be erased/moved): '%s'" % (a_path,)
                self.write(temp)
            ...
            if os.path.exists(a_path) or os.path.islink(a_path):
                if os.path.isdir(a_path):
                    temp = "==> Erasing directory (and all directories and files within): '%s'" %(a_path,)
                    self.write(temp)
                    shutil.rmtree(a_path)
                    if a_path in self.temp_paths:
                        self.temp_paths.remove(a_path)
                elif os.path.isfile(a_path) or os.path.islink(a_path):
                    temp = "==> Erasing file: '%s'"%(a_path,)
                    self.write(temp)
                    os.remove(a_path)
                    if a_path in self.temp_paths:
                        self.temp_paths.remove(a_path)
                elif a_path in self.__devs:
                    temp = "==> Pretending to erase the file: '%s'"%(a_path,)
                    self.write(temp)
                    if a_path in self.temp_paths:
                        self.temp_paths.remove(a_path)
                else:
                    temp = "'%s' is not a directory nor a file. It is not deleted!" % (a_path,)
                    self.write(temp)
    ...
    def clean(self, a_path, temp_path = 'yes'):
        ...
        self.__show_step_header_start()
        if temp_path == 'yes':
            self.write("Erasing '%s'" %(a_path,))
        elif temp_path == 'no':
            self.write("Skipping from erasing '%s'" %(a_path,))
        else:
